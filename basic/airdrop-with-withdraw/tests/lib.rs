...
fn try_withdraw_without_added_recipients_must_be_failed() {
    ...
    assert_eq!(test_env.get_balance(test_env.admin_account, RADIX_TOKEN).unwrap(), dec!(1000000));
...
fn try_withdraw_after_added_recipients_must_be_succeeded() {
    ...
    assert_eq!(test_env.get_balance(test_env.admin_account, RADIX_TOKEN).unwrap() , dec!(1000000) - token_by_recipient * recipient_count);
    ...
    for (recipient_key, recipient_address)  in recipients_accounts {
           // available token
            ...
            assert_eq!(test_env.get_balance(recipient_address, RADIX_TOKEN).unwrap(), dec!(1000000) + token_by_recipient);
    }
}
...
impl<'a> TestEnv<'a> {
    pub fn new(ledger: &'a mut InMemorySubstateStore, token_type : Address) -> Self {
        ...
        let tx = TransactionBuilder::new(&executor)
            .call_function(package.unwrap(), "AirdropWithWithdraw", "new", vec![token_type.to_string()], Some(admin_account))
    ...
    pub fn add_recipient(&mut self, recipient: Address, token_address: Address, tokens: Decimal) -> radix_engine::model::Receipt {
