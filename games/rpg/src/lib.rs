...
blueprint! {
    ...
    impl Substradix {
        ...
        pub fn level_up_class2(&mut self, nft_proof: Proof) {
            assert!(
                nft_proof.amount() == dec!("1"),
                "You can only level up one character at once!"
            );
        ...
        pub fn level_up_class3(&mut self, nft_proof: Proof) {
            assert!(
                nft_proof.amount() == dec!("1"),
                "You can only level up one character at once!"
            );
        ...
        pub fn level_up_class4(&mut self, nft_proof: Proof) {
            assert!(
                nft_proof.amount() == dec!("1"),
                "You can only level up one character at once!"
            );
        ...
        pub fn level_up_class5(&mut self, nft_proof: Proof) {
            assert!(
                nft_proof.amount() == dec!("1"),
                "You can only level up one character at once!"
            );
            ...
            if speed < enemy_speed {
                loop{
                    ...
                    if health <= 1 {
                        ...
                        self.system_vault.authorize(|| nft_proof.non_fungible().update_data(nft_data));
                        match class {
                            1 => { self.level_up_class1(nft_proof); },
                            2 => { self.level_up_class2(nft_proof); },
                            3 => { self.level_up_class3(nft_proof); },
                            4 => { self.level_up_class4(nft_proof); },
                            5 => { self.level_up_class5(nft_proof); },
                            _ => return
                        };
                        self.system_vault.put(key_bucket);
                        return
